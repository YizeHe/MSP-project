...
**English** | **[中文](guide-cn.md)** ← 当前  
切换：[English guide](guide.md)
...
默认 `./msp-data`（`MSP_DATA` 可覆盖）：`identity.json`、`config.json`、`trust.json`、`mst-ledger.json`、`chain/` 等。

**切勿**把 `msp-data*` 提交到 Git。助记词请离线备份；丢失无法恢复。
...
| 现象 | 处理 |
|------|------|
| `POST /v1/hello` 400 / 旧 seed | 换新 `msp-seed`；health 应为 `0.2.0-test` + `signaling-only` |
| `neighbors 0` | 检查 UDP 防火墙、种子、`discover` |
| `supply exhausted` | 210 名额用尽 → `chain-mine` |
| `insufficient` | `chain-claim` 或挖矿 |
| ticket 失败 | 链同步；或实验室 `MSP_REQUIRE_TICKET=0` |
| 出块太慢 | `MSP_POW_FAST=1` `MSP_CHAIN_FAST=1` |
...
| 路径 | 说明 |
|------|------|
| `Guide/guide.md` | 英文使用指南（默认） |
| `Guide/guide-cn.md` | **本文件**（中文） |
| `README.md` | 英文总览 |
| `README-cn.md` | 中文总览 |
| `LICENSE` | MIT |
| `代办.md` … `代办7.md` | 迭代规格 |
